        ...
        <div class="pad">
            <div class="pad-title">日志管理
                <el-popover
                    ...
                    width="520"
                    trigger="click">
                    <el-form :inline="true" :model="formInline" label-width="70px" class="demo-form-inline" style="text-align: left;margin-top: 25px;">
                        <el-form-item label="业务名称" style="margin-left: 25px">
                            <el-input v-model="formInline.name" @keyup.native="validateX" @keydown.native="validateX" style="width: 400px;" placeholder="请输入业务名称"></el-input>
                        </el-form-item>
                        <el-form-item label="任务ID" style="margin-left: 25px">
                            <el-input v-model="formInline.taskId" @keyup.native="validateX" @keydown.native="validateX" style="width: 400px;" placeholder="请输入任务ID"></el-input>
                        </el-form-item>
                        <el-form-item label="添加时间" style="margin-left: 25px;">
                            <el-date-picker
                        ...
                        <el-form-item label="日志内容" style="margin-left: 25px">
                            <el-input type="textarea" v-model="formInline.logContent" @keyup.native="validateX" @keydown.native="validateX" style="width: 400px;" placeholder="请输入日志内容"></el-input>
                        </el-form-item>
    ...
    export default {
        ...
        methods: {
            ...
            beforeUpload(file) {
                let fd = new FormData();
                fd.append("file", file);
                this.$api.decision.uploadPlanLog.request(fd).then((data) => {
                    console.log(data);
                    if (data.data) {
                        this.$message({
                            showClose: true,
            ...
            downLoads() {
                ...
                this.$api.decision.downLoadPlanLog.request({id: idsArr}).then((data) => {
                    this.tableList();
            ...
            // 下载全部的接口
            downLoadsAll() {
                this.$api.decision.downLoadPlanLog.request({id: []}).then((data) => {
                    this.tableList();
                    this.downloadFiles(data.data);
                }).catch((err) => {
                    this.$message.error(err.response.data.detail);
            ...
            tableList() {
                ...
                this.$api.decision.planLog.request({},
                    {restUrl: `page=${this.currentPage1 - 1}&&size=${this.pageSize}&&name=${this.formInline['name']}&&addTime=${tt}&&endTime=${tt2}&&taskId=${this.formInline.taskId}&&content=${this.formInline.logContent}`})
                    .then((data) => {
                    this.tableData = data.data.logList;
                    this.total1 = data.data.logCount;
                    this.transform(data);
                    setTimeout(() => {
                        this.setSelectRow();
                    }, 20);
            ...
            selectLog() {
                ...
                this.currentPage1 = 1;
                this.$api.decision.selectPlanLog.request({},
                    {restUrl: `page=${0}&&size=${this.pageSize}&&name=${this.formInline['name']}&&addTime=${tt}&&endTime=${tt2}&&taskId=${this.formInline.taskId}&&content=${this.formInline.logContent}`})
                    .then((data) => {
                    this.tableData = data.data.logList;
                    this.total1 = data.data.logCount;
                    this.transform(data);
            ...
            transform(data) {
                for (let i = 0, length = data.data.logList.length; i < length; i++) {
                    ...
                    if (this.tableData[i]['logcontent']) {
                        this.tableData[i]['logcontent'] = this.tableData[i]['logcontent'].split('"')[1];
                    }
                }
            },
            // 关闭弹窗时清空表单数据
