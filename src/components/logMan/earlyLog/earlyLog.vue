            ...
            <div class="pad-title">日志管理
                <el-popover
                    ...
                    <el-form :inline="true" :model="formInline" label-width="70px" class="demo-form-inline" style="text-align: left;margin-top: 25px;">
                        <el-form-item label="生成时间" style="margin-left: 30px;">
                            <el-date-picker
    ...
    export default {
        ...
        methods: {
            ...
            beforeUpload(file) {
                let fd = new FormData();
                fd.append("file", file);
                this.$api.decision.uploadEarlyLog.request(fd).then((data) => {
                    if (data.data) {
                        this.$message({
                            showClose: true,
                            message: '上传成功',
            ...
            downLoads() {
                ...
                this.$api.decision.downloadEarlyLog.request({id: idsArr}).then((data) => {
                    this.tableList();
            ...
            // 下载全部的接口
            downLoadsAll() {
                this.$api.decision.downloadEarlyLog.request({id: []}).then((data) => {
                    this.tableList();
                    this.downloadFiles(data.data);
                }).catch((err) => {
                    this.$message.error(err.response.data.detail);
            ...
            tableList() {
                ...
                this.$api.decision.earlyLog.request({},
                    {restUrl: `page=${this.currentPage1 - 1}&&size=${this.pageSize}&&addTime=${tt}&&endTime=${tt2}`})
                    .then((data) => {
            ...
            selectLog() {
                ...
                this.currentPage1 = 1;
                this.$api.decision.earlyLog.request({},
                    {restUrl: `page=${0}&&size=${this.pageSize}&&addTime=${tt}&&endTime=${tt2}`})
                    .then((data) => {
                    this.tableData = data.data.data;
                    this.total1 = data.data.total;
                    this.transform(data);
            ...
            transform(data) {
                for (let i = 0, length = data.data.total; i < length; i++) {
                    if (this.tableData[i]['lastSyncTime']) {
                        this.tableData[i]['lastSyncTime'] = this.tableData[i]['lastSyncTime'].split('T')[0] +
                            " " + this.tableData[i]['lastSyncTime'].split('T')[1].split('+')[0];
                    }
                }
            },
            // 关闭弹窗时清空表单数据
            ...
            earlyLogState() {
                this.$api.decision.earlyLogState.request(
                    {
                        userName: 1,
                        updateTime: 2,
                        remarks: 3
